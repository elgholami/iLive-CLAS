...
{
  static TypeId tid = TypeId ("ns3::sixlowpan::SixLowPanNetDevice")
      .SetParent<NetDevice> ()
                                            ...
                                            PacketType packetType)
{
  ...
  if( dispatchVal == SixLowPanDispatch::LOWPAN_MESH )
    {
      NS_FATAL_ERROR("Unsupported 6LoWPAN encoding: MESH, exiting.");
    }

  if( dispatchVal == SixLowPanDispatch::LOWPAN_BC0 )
    {
      NS_FATAL_ERROR("Unsupported 6LoWPAN encoding: BC0, exiting.");
    }
                               ...
                               uint16_t protocolNumber)
{
  ...
  origHdrSize += CompressLowPanHc1(packet, m_port->GetAddress(), dest, headersPre);

  if( packet->GetSerializedSize() > GetMtu() )
    {
      // fragment
      std::list<Ptr<Packet> > fragmentList;
      DoFragmentation(packet, origPacketSize, origHdrSize, headersPre, headersPost, fragmentList);
  ...
  if( packet->PeekHeader(ipHeader) != 0 )
    {
      ...
      Ipv6Address mySrcAddr = Ipv6Address::MakeAutoconfiguredLinkLocalAddress(Mac48Address::ConvertFrom(src));
      ...
      Ipv6Address myDstAddr = Ipv6Address::MakeAutoconfiguredLinkLocalAddress(Mac48Address::ConvertFrom(dst));
                                         ...
                                         Ptr<HeaderStorage> headers)
{
  ...
  switch(encoding.GetSrcCompression())
    ...
    case SixLowPanHc1::HC1_PIIC:
      ...
      ipHeaderPtr->SetSourceAddress( Ipv6Address::MakeAutoconfiguredAddress(Mac48Address::ConvertFrom(src),
                                                                        Ipv6Address(address)) );
      break;
    ...
    case SixLowPanHc1::HC1_PCIC:
      ipHeaderPtr->SetSourceAddress( Ipv6Address::MakeAutoconfiguredLinkLocalAddress(Mac48Address::ConvertFrom(src)) );
      break;
    }
  ...
  switch(encoding.GetDstCompression())
    ...
    case SixLowPanHc1::HC1_PIIC:
      ...
      ipHeaderPtr->SetDestinationAddress( Ipv6Address::MakeAutoconfiguredAddress(Mac48Address::ConvertFrom(dst),
                                                                        Ipv6Address(address)) );
      break;
    case SixLowPanHc1::HC1_PCII:
      interface = encoding.GetDstInterface();
      ...
      break;
    case SixLowPanHc1::HC1_PCIC:
      ipHeaderPtr->SetDestinationAddress( Ipv6Address::MakeAutoconfiguredLinkLocalAddress(Mac48Address::ConvertFrom(dst)) );
      break;
    }
...
void SixLowPanNetDevice::FinalizePacketPreFrag(Ptr<Packet> packet, Ptr<HeaderStorage> headers)
{
  if(headers->IsEmpty())
    {
...
void SixLowPanNetDevice::FinalizePacketPostFrag(Ptr<Packet> packet, Ptr<HeaderStorage> headers)
{
  // MESH and BC0
  return;
}

void SixLowPanNetDevice::FinalizePacketIp(Ptr<Packet> packet, Ptr<HeaderStorage> headers)
{
  if(headers->IsEmpty())
    {
       return;
...
void SixLowPanNetDevice::DoFragmentation (Ptr<Packet> packet,
                                          uint32_t origPacketSizexxx,
                                          uint32_t origHdrSize,
                                          Ptr<HeaderStorage> headersPre,
                                          ...
                                          std::list<Ptr<Packet> >& listFragments)
{
  ...
  uint32_t size;
  NS_ASSERT_MSG( l2Mtu > frag1Hdr.GetSerializedSize() + cmpHdrSizePre + cmpHdrSizePost,
                 "6LoWPAN: can not fragment, 6LoWPAN headers are bigger than MTU");

  size = (l2Mtu - frag1Hdr.GetSerializedSize() - cmpHdrSizePre - cmpHdrSizePost) & 0x7;
      ...
      if( m_fragmentReassemblyListSize && (m_fragments.size() >= m_fragmentReassemblyListSize) )
        {
          ...
          m_dropTrace (DROP_FRAGMENT_BUFFERFULL, m_node->GetObject<SixLowPanNetDevice> (), GetIfIndex());
        }
...
void SixLowPanNetDevice::HeaderStorage::StoreHeader(TypeId headerType, Header* header)
{
...
void SixLowPanNetDevice::HandleFragmentsTimeout (FragmentKey key, uint32_t iif)
{
  ...
  m_dropTrace (DROP_FRAGMENT_TIMEOUT, m_node->GetObject<SixLowPanNetDevice> (), iif);
