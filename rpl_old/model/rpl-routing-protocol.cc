...
void RoutingProtocol::SendPeriodicDIOPacket(Ipv6Address src, Ipv6Address dst, Address* hardwareAddress, uint8_t flags, rpl_instance_t *instance) {
	...
	NS_LOG_LOGIC ("Send DIO ( from " << src << " to " << dst << " target " << src << ")");
	dio.SetIpv6Target(src);
...
//	SendMessage(p, src, dst, 255);
	...
	if (dst.IsAllNodesMulticast()) {
		std::cout << "RPL: Sending a multicast-DIO with rank %u\n"
...
void RoutingProtocol::SetIpv6(Ptr<Ipv6> ipv6) {
	...
	NS_ASSERT(
			m_ipv6->GetNInterfaces () == 1 && m_ipv6->GetAddress (0, 0).GetAddress() == Ipv6Address ("fe80::1"));
...
void RoutingProtocol::RPLSend(Ptr<Ipv6Route> route, Ptr<const Packet> packet,
		const Ipv6Header & header) {
...
//	dag = get_dag(instance_id, dag_id);
	...
	if (dag == NULL) {
		std::cout << "RPL: Failed to allocate a DAG\n" << std::endl;
		return NULL;
	}
