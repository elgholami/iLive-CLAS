 ...
 */
#include "ipv6-olsr-helper.h"
#include "ns3/ipv6-olsr-routing-protocol.h"
...
{
  m_agentFactory.SetTypeId ("ns3::ipv6_olsr::RoutingProtocol");
}
...
{
  Ptr<ipv6_olsr::RoutingProtocol> agent = m_agentFactory.Create<ipv6_olsr::RoutingProtocol> ();
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      ...
      Ptr<ipv6_olsr::RoutingProtocol> olsr = DynamicCast<ipv6_olsr::RoutingProtocol> (proto);
      if (olsr)
        {
      ...
      if (list)
        {
          ...
          Ptr<ipv6_olsr::RoutingProtocol> listOlsr;
          for (uint32_t i = 0; i < list->GetNRoutingProtocols (); i++) 
            {
              listProto = list->GetRoutingProtocol (i, priority);
              listOlsr = DynamicCast<ipv6_olsr::RoutingProtocol> (listProto);
              if (listOlsr)
                {
