...
int main (int argc, char *argv[])
{
  ...
  Ptr<ipv6_olsr::RoutingProtocol> olsrrp_Gw;

  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
    {
      int16_t priority;
      Ptr<Ipv4RoutingProtocol> temp = lrp_Gw->GetRoutingProtocol (i, priority);
      if (DynamicCast<ipv6_olsr::RoutingProtocol> (temp))
        {
          olsrrp_Gw = DynamicCast<ipv6_olsr::RoutingProtocol> (temp);
        }
    }
  ...
  // Tracing
  wifiPhy.EnablePcap ("ipv6-olsr-hna", devices);
  csma.EnablePcap ("ipv6-olsr-hna", csmaDevices, false);
