 ...
 */
UdpHeader::UdpHeader() :
		m_sourcePort(0xfffd), m_destinationPort(0xfffd), m_payloadSize(0xfffd), m_calcChecksum(
				false), m_goodChecksum(true) {
}
UdpHeader::~UdpHeader() {
	m_sourcePort = 0xfffe;
...
void UdpHeader::EnableChecksums(void) {
	m_calcChecksum = true;
}
...
void UdpHeader::SetPayLoadSize(uint16_t payloadSize) {
	m_payloadSize = payloadSize;
}
void UdpHeader::SetSourcePort(uint16_t port) {
	m_sourcePort = port;
}
uint16_t UdpHeader::GetSourcePort(void) const {
	return m_sourcePort;
}
uint16_t UdpHeader::GetDestinationPort(void) const {
...
TypeId UdpHeader::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::UdpHeader").SetParent<Header>().AddConstructor<
					UdpHeader>();
	return tid;
}
TypeId UdpHeader::GetInstanceTypeId(void) const {
	return GetTypeId();
}
...
uint32_t UdpHeader::GetSerializedSize(void) const {
	return 8;
}
...
void UdpHeader::Serialize(Buffer::Iterator start) const {
	...
	if (m_calcChecksum) {
		...
		i = start;
		uint16_t checksum = i.CalculateIpChecksum(start.GetSize(),
				headerChecksum);
...
uint16_t UdpHeader::GetChecksums(void) const {
	return m_checksum;
}
